...
//#pragma omp parallel for num_threads(thread_count)
    ...
    float pre_changed = 0;
    float cur_changed = 0;
//#pragma omp parallel for num_threads(thread_count) reduction(+:cur_changed)
    for(int i = 0; i < num_samples; i ++){
...
#pragma omp parallel num_threads(thread_count)
        {
#pragma omp for private(j,k) schedule(guided ,512)
            for (i = 0; i < num_samples; ++i) {
                ...
                for (j = 0; j < num_classes; ++j) {
                    tmp[i][j] = 0.0;
                    sum = vdupq_n_f32(0.0);
                    for (k = num_samples - 4; k >= 0; k -= 4) {
                        t1 = vld1q_f32(affinity_matrix[i] + k);
                        t2 = vld1q_f32(label_function_T[j] + k);
                        t1 = vmulq_f32(t1, t2);
                    ...
                    s1 = vpadd_f32(s1, s2);
                    s1 = vpadd_f32(s1, s1);
                    vst1_lane_f32(tmp[i] + j, s1, 0);
                    // 计算剩余的四个元素
                    for (k = (num_samples % 4) - 1; k >= 0; --k) {
        ...
        //#pragma omp parallel for reduction(+:cur_changed)
        for( i = 0; i < num_samples; i ++){
            float32x4_t t_pre,t_cur;
            float32x4_t res_change = vmovq_n_f32(0);
            for( j = num_classes - 4; j >= 0; j -=4){
                t_pre = vld1q_f32(pre_label_function[i] + j);
                t_cur = vld1q_f32(label_function[i] + j);
                t_pre = vsubq_f32(t_pre,t_cur);
                res_change = vaddq_f32(res_change, vabsq_f32(t_pre));
